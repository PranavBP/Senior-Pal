...
// import 'package:hero_minds/controllers/home_page_controller.dart';
...
class _HomePageState extends ConsumerState<HomePage> {
  ...
  void initState() {
    super.initState();
    _homeController = HomePageController(ref);
    _initializeData();
  }
  ...
  Future<void> _initializeData() async {
    ...
    final showPopup = await _homeController.shouldShowPopup();
    if (showPopup) {
      await _showDailyQuotePopup();
      _homeController.handleDailyCheckInPopup(_showDailyCheckInPopup);
    }
  }

  Future<void> _fetchData() async {
    final data = await _homeController.fetchUserData();
    setState(() {
      ...
      await showDialog(
        ...
        builder: (BuildContext context) {
          return DailyQuotePopup(
            quote: _dailyQuote,
            onClose: () async {
              await _homeController.updateLastSeenDate();
        ...
        children: [
          ...
          Positioned.fill(
            top: MediaQuery.of(context).size.height *
                0.5, // Start content below the background
            child: Container(
              color: Colors.transparent, // Ensure no background interference
              child: Scrollbar(
                child: SingleChildScrollView(
                  padding: const EdgeInsets.all(8.0),
                  child: Column(
                    ...
                    children: [
                      ...
                      AnimatedSwitcher(
                        ...
                        child: HomeQuote(quote: '💡 $_dailyQuote'),
                      ),
                      const SizedBox(height: 16),
                      const HomeButtons(),
                    ],
